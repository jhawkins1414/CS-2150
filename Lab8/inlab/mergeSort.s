...
; Return type is void 
mergeSort:
	cmp rsi, rdx		; compare left element and right element
	jge done		; if left > right, done
	push rsi		; left element
	push rdx		; right element
	add rdx, rsi		; add left to right
	shr rdx, 1		; divide sum by 2
	push rdx		; sum / 2
	call mergeSort		; recurse
	
	pop rsi			; middle of array goes to left
	pop rdx			; right goes to right
	inc rsi 		; inc left
	push rdx		; store right
	call mergeSort		; recurse
	pop rdx			; right goes to left
	pop rsi			; left goes to left
	mov rcx, rdx		; right goes to next parameter
	add rdx, rsi		; adding left to right
	shr rdx, 1		; divide sum by 2
